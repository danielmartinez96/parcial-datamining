...
lm_sin_condicion <- lm(formula = price ~ . -id, data = select(precio_lot2, -c("date", "id", "condition","sqft_basement","sqft_above","sqft_price","lat","long","sqft_living","sqft_lot","yr_built","yr_renovated","view","waterfront")))
summary(lm_sin_condicion)
par(mfrow=c(2,2))
plot(lm_sin_condicion)
...
clean_data= select(a_raw_data, -c("long","lat","id"))
...
clean_data= select(clean_data,-sqft_living,-sqft_lot, -date, -id, -sqft_above, -sqft_basement
                   ,-yr_built, -sqft_lot15, -waterfront)

#outliers

clean_data= filter(clean_data,sqft_living15 > 1000 & sqft_living15 < 3500)
clean_data= filter(clean_data,price > 100000 & price < 1000000 )
clean_data= filter(clean_data, bedrooms >= bathrooms )

clean_data = mutate(clean_data, bedrooms = ceiling(bedrooms))
clean_data = mutate(clean_data, bathrooms = ceiling(bathrooms))
clean_data = mutate(clean_data, floors = ceiling(floors))
clean_data = mutate(clean_data, fue_renovada = ifelse(yr_renovated>0, 1 , 0))

modelo_clean<-lm(price~.,data=select(clean_data, -c("id", "bedrooms", "yr_renovated", "zipcode")))
...
```{r}
as <- clean_data %>% mutate(asd = sqft_living15 / floors) %>% select(-c("id","grade","zipcode"))
modelo_clean_1<-lm(price~ .,data= as)
summary(modelo_clean_1)
plot(modelo_clean_1)
hist(modelo_clean_1$residuals)
```
...
#Precio/Sqft
clean_data=filter(clean_data,price/living15>13)
clean_data=filter(clean_data,price/living15<500)
...
## 7.2 Regresiones lineales

```{r Regresiones de price contra todos}
summary(lm(price~bedrooms,data=modelo_clean))
```
```{r }
summary(lm(price~bathrooms,data=modelo_clean))
```

```{r }
summary(lm(price~sqft_living,data=modelo_clean))
```

```{r }
summary(lm(price~sqft_lot,data=modelo_clean))
```

```{r }
summary(lm(price~floors,data=modelo_clean))
```

```{r }
summary(lm(price~waterfront,data=modelo_clean))
```

```{r }
summary(lm(price~view,data=modelo_clean))
```

```{r }
summary(lm(price~condition,data=modelo_clean))
```

```{r }
summary(lm(price~grade,data=modelo_clean))
```

```{r }
summary(lm(price~sqft_above,data=modelo_clean))
```

```{r }
summary(lm(price~sqft_basement,data=modelo_clean))
```

```{r }
summary(lm(price~yr_built,data=modelo_clean))
```

```{r }
summary(lm(price~yr_renovated,data=modelo_clean))
```

```{r }
summary(lm(price~zipcode,data=modelo_clean))
```

```{r }
summary(lm(price~lat,data=modelo_clean))
```

```{r }
summary(lm(price~long,data=modelo_clean))
```

```{r }
summary(lm(price~sqft_living15,data=modelo_clean))
```


```{r }
summary(lm(price~sqft_lot15,data=modelo_clean))
```

```{r }
summary(lm(price~bedbat,data=modelo_clean))
```

```{r }
summary(lm(price~antiguedad,data=modelo_clean))
```

sqft_lot15
```{r }
as <- clean_data %>% mutate(asd = sqft_living15 / floors) %>% select(-c("id","grade", "lat", "long", "sqft_lot15")) 
modelo_clean_1<-lm(price~ .,data= as)
summary(modelo_clean_1)
plot(modelo_clean_1)
hist(modelo_clean_1$residuals)
