...
year = lubridate::year(df_sin_condition$date)
month = lubridate::month(df_sin_condition$date)

df_fecha_form = df_sin_condition %>% mutate(date_year = as.factor(paste(year,month, sep="/")))
```
...
```{r}
plot(df_fecha_form$price ~ df_fecha_form$date_year)
```

```{r}
df_sin_date = select(df_sin_condition, -c(id,date))
```
